...
{
	// This is used to wait for all worker threads to finish thier tasks before the next frame:
	update_barrier = new boost::barrier(WORKER_THREADS + 1); // Worker threads + the main thread.
	// Pass a pointer to this object to the thread manager:
	thread_manager = new ThreadManager(update_barrier, WORKER_THREADS); // Initialises thread manager class with the passed in number of threads.
	// You can then add tasks to this and it will devide work among the other threads.
	...
	text_money.setColor(sf::Color::Green);

	text_health.setFont(font);
	text_health.setColor(sf::Color::Red);
...
// Returns a copy of the grid, so that seperate threads can work on the same data without blocking:
Grid Game::CopyGrid()
{
	Grid graph_copy(26, std::vector<Vertex*>(20));
...
{
	ScheduledDeletion(); // This is better than deleting objects mid-loop.
	...
	{
		auto shot = dynamic_cast<Shot*>(object);
		thread_manager->AddTask(new ShotUpdateTask(shot, dt));
	}
	// For each cell:
	...
	update_barrier->wait(); // This will wait for all the tasks to complete before continuing.
	window.display();
}
...
void Game::Draw()
{
	...
	for (int i = 0; i < 520; i++)
	{
		...
		{
			thread_manager->Signal(); 
		}
		auto square = square_channel.Read(); // This will wait for data to become avialable from the update cell tasks.
		window.draw(*square); 
	}
	// Draw game objects:
	for (auto tank : tanks)
...
// Updates an individual squares data, also handles detection of clicks on squares to spawn/remove turrets:
void Game::UpdateSquare(int x, int y)
{
	...
	// Task then determines user input on the square:
...
// Calculates a respawn time based on number destroyed:
