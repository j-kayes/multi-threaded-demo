...
Tank::Tank(sf::Texture &texture, Game* game_ptr, ThreadManager* manager): thread_manager(manager),
	health(kTankInitialHealth), speed(kTankSpeed), movement_direction(RIGHT), cell_x(-1), cell_y(9)
{
...
void Tank::UpdateDirection()
{
	...
	lock.unlock();
	// Calculate direction from current to next node to get direction:
  	if ((next_node->coordinates_.x - current_node->coordinates_.x) > 0)
	{
...
void Tank::SetPath(std::list<Vertex*> path)
{
